...
@section('content')
  <!--begin::Card-->
  <form action="{{ route('bidang.update', ['bidang' => $data->id]) }}" class="row" method="POST">
    @csrf
    @method('PUT')
    <div class="col-md-12">
      <div class="card card-custom card-stretch gutter-b">
        <div class="card-header">
          <h3 class="card-title">Ubah Bidang</h3>
        </div>
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <x-validation.txt-stack type="text" id="b_name" name="b_name" placeholder="Nama Bidang"
                value="{{ old('b_name') ?? $data->b_name }}" :messages="$errors->get('b_name')">Nama Bidang
                <x-redstar /></x-validation.txt-stack>

              <x-validation.txt-stack type="text" id="b_kode" name="b_kode" placeholder="Kode Bidang"
                value="{{ old('b_kode') ?? $data->b_kode }}" :messages="$errors->get('b_kode')">Kode Bidang
                <x-redstar /></x-validation.txt-stack>
