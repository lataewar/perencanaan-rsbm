...
@section('content')
  <!--begin::Card-->
  <form action="{{ route('user.update', ['user' => $data->id]) }}" class="row" method="POST">
    @csrf
    @method('PUT')
    <div class="col-md-12">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <x-validation.txt-stack type="text" id="name" name="name" placeholder="Nama User"
                value="{{ old('name') ?? $data->name }}" :messages="$errors->get('name')">Nama User
                <x-redstar /></x-validation.txt-stack>

              <x-validation.txt-stack type="email" id="email" name="email" placeholder="Email"
                value="{{ old('email') ?? $data->email }}" :messages="$errors->get('email')">Email
                <x-redstar /></x-validation.txt-stack>
              ...
              <x-validation.select-static-stack name="role_id" id="role_id" :messages="$errors->get('role_id')">
                ...
                @slot('current', old('role_id') ?? $data->role_id->value)
              </x-validation.select-static-stack>

              <div id="unit" @if ($data->role_id->value != 5) style="display: none;" @endif>
                <x-validation.select-static-stack name="unit_id" :messages="$errors->get('unit_id')">
                  Role <x-redstar />
                  @slot('items', $units)
                  @slot('current', old('unit_id') ?? $data->unit_id)
                </x-validation.select-static-stack>
              </div>

              <div id="bidang" @if ($data->role_id->value !=6) style="display: none;" @endif>
                <x-validation.select-static-stack name="bidang_id" :messages="$errors->get('bidang_id')">
                  Bidang
                  <x-redstar />
                  @slot('items', $bidangs)
                  @slot('current', old('bidang_id') ?? $data->bidang_id)
                </x-validation.select-static-stack>
              </div>
